...
/**
 在Reset时是否清除画布内容。
 */
@property(nonatomic) BOOL clearCanvasWhenReset;
...
/**
 ...
 @param isEnable 传入YES时会开启循环播放。
*/
- (void)enableLooping:(BOOL)isEnable;
...
/**
 ...
 @param isEnabe 是否开启。
*/
- (void)toggleFrameChasing:(BOOL)isEnabe;

/**
 设置连接服务器与下载数据过程中的超时时间，需要在调用prepare之前执行
 
 @param timeout 超时时间，单位为微秒
 */
- (void)setTimeoutInUs:(int)timeout;
...
/**
 设置播放器的一些内部选项，一般不用使用到。
 */
@interface BDCloudMediaPlayerController (Options)
...
- (void)setOptionValue:(NSString *)value
                forKey:(NSString *)key
            ofCategory:(BDCloudMediaPlayerOptionCategory)category;
...
@interface BDCloudMediaPlayerController (Subtitle)

/**
 设置是否解码字幕Track。 默认不解码。如果打开此功能，字幕会通过 BDCloudMediaPlayerTimedTextNotification 通知给到用户。
 
 @param enable 是否解码。
 */
- (void)setEnableDecodeSubtitle:(BOOL)enable;
...
// 字幕
BDCLOUD_MP_EXTERN NSString* const BDCloudMediaPlayerTimedTextNotification;
...
// 字幕，用于 BDCloudMediaPlayerTimedTextNotification 中。
BDCLOUD_MP_EXTERN NSString* const BDCloudMediaPlayerTimedTextUserInfoKey;
